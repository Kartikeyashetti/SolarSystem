...
export default function Planet({
  ...
  const planetRef = useRef(); // Single ref for both animation and click
  const [angle, setAngle] = useState(() => Math.random() * Math.PI * 2);

  const texture = useLoader(THREE.TextureLoader, textureUrl);

  // Animate orbit and self-spin
  useFrame(() => {
  ...
  const orbitPoints = useMemo(() => {
    ...
    const points = curve.getPoints(100).map(p =>
      new THREE.Vector3(p.x + center[0], center[1], p.y + center[2])
    );
  ...
  return (
    <>
      ...
      <mesh
        ref={planetRef}
        onClick={() => onPlanetClick?.(planetRef.current.position,name)}
      >
